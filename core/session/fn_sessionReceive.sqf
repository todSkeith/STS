...
switch (playerSide) do
{
	...
	case civilian:
	{
		...
		[[getPlayerUID player, playerSide],"BRUUUDIS_fnc_queryPlayerHouses",false,false] spawn life_fnc_MP; //fetches houses
	};
	...
	case independent:
	{
		...
		life_cash = parseNumber(_session select 2);
		life_atmcash = parseNumber(_session select 3);
		__CONST__(life_adminlevel,parseNumber(_session select 4));
		__CONST__(life_donator,parseNumber(_session select 5));
		__CONST__(life_medicLvl,parseNumber(_session select 6));
		__CONST__(life_coplevel,0);
	};
};
